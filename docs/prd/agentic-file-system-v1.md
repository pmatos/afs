...
- **Status (PRD #2 supersession of the runtime adapter wire format)**: PRD #1's seam (subprocess RPC) holds. The hand-rolled tab-delimited protocol described in the v1 README and implemented in v1's tests is replaced by Pi's documented JSONL JSON-RPC plus a vendored `afs_reply` Pi extension that pins structured output via a TypeBox schema. See `docs/prd/agentic-file-system-v2-pi-rpc-rewrite.md` and `docs/adr/0002-pi-jsonl-rpc-adapter.md`. The adapter rewrite addresses issue #40, where real Pi rejected every AFS request because v1's protocol existed only in AFS's in-tree fake.
