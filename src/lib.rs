...
pub mod supervisor {
    use crate::history_backend::{HistoryBackend, HistoryEntry};
    ...
    fn handle_client(mut stream: UnixStream, runtime: &SupervisorRuntime) -> io::Result<()> {
        ...
        if let Some(payload) = request.strip_prefix("ASK\t") {
    ...
    impl SupervisorRuntime {
        ...
        fn perform_delegated_queued_task(
            ...
            let nested = ManagedSubtree::new(&target.managed_dir, &target.agent_home)
                .nested_managed_relative_paths()?;
            if let Some(change) = target
                .history
                .record_agent_change(&target.managed_dir, &nested)?
            {
    ...
    impl SupervisorState {
        fn install(&mut self, path: &Path) -> io::Result<String> {
            ...
            {
                let install_subtree = ManagedSubtree::new(&managed_dir, &agent_home);
                let nested = install_subtree.nested_managed_relative_paths()?;
                HistoryBackend::open(&agent_home)?.record_baseline(&managed_dir, &nested)?;
            }
            ...
            if !was_already_managed && let Some(parent_agent_index) = parent_agent_index {
                ...
                let parent_nested = parent_subtree.nested_managed_relative_paths()?;
                parent.history.record_ownership_event(
                    &parent.managed_dir,
                    &parent_nested,
        ...
        fn remove_nested(
            ...
            let parent_history = self.agents[parent_agent_index].history.clone();
            ...
            let outcome_result: io::Result<(PathBuf, RemoveOutcome)> = (|| {
                ...
                let child_origin = ManagedSubtree::new(&parent_managed_dir, &parent_agent_home)
                    .relative_path(&managed_dir)?;

                // Recompute parent_nested AFTER any archive/discard so we
                // mirror legacy semantics: the post-archive Ownership Merge
                // commit must stage files newly uncovered by removing the
                // child's .afs/ (otherwise transitively-replayed history loses
                // those files).
                if !removed_agent_home.exists() {
                    let parent_nested =
                        ManagedSubtree::new(&parent_managed_dir, &parent_agent_home)
                            .nested_managed_relative_paths()?;
                    parent_history.record_ownership_event(
                        &parent_managed_dir,
                        &parent_nested,
                ...
                let parent_nested = ManagedSubtree::new(&parent_managed_dir, &parent_agent_home)
                    .nested_managed_relative_paths()?;
                parent_history.record_ownership_event(
                    &parent_managed_dir,
                    &parent_nested,
                ...
                parent_history.merge_archived_child_history(
                    &archived_agent_home,
                    &parent_managed_dir,
                    &child_origin,
                )?;
        ...
        fn history(&self, path: &Path) -> io::Result<String> {
            ...
            let entries = self.agents[agent_index].history.entries()?;
        ...
        fn undo(
            ...
            let nested = ManagedSubtree::new(&agent.managed_dir, &agent.agent_home)
                .nested_managed_relative_paths()?;
            let outcome =
                agent
                    .history
                    .undo_latest(&agent.managed_dir, &nested, history_entry, confirmed)?;
            Ok(format!(
                "undid history entry {}\nfiles={}\n",
                outcome.entry_id,
                outcome.files.len()
        ...
        fn stream_broadcast_ask(
            ...
            response.emit_progress(format_args!(
                "route=broadcast agents={n} timeout_ms={}",
                timeout.as_millis()
            ))?;
            if n == 0 {
            ...
            let replies: Vec<BroadcastReply> = indexed_replies
                ...
                .map(|(_, reply)| reply)
                .collect();
            response.write_body(&format_broadcast_ask_response(&replies, &outcome, timeout))
        }
        ...
        fn load_registry(&mut self) -> io::Result<()> {
            ...
            for line in registry.lines().skip(1) {
                ...
                let reconciliation_files = {
                    ...
                    HistoryBackend::open(&agent_home)?.pending_external_files(
                        &managed_dir,
                        &nested,
                        SystemTime::now(),
                    )?
        ...
        fn start_registered_agent(
            ...
            let history = Arc::new(HistoryBackend::open(&agent_home)?);
            let monitor = start_directory_monitor(
                managed_dir.clone(),
                agent_home.clone(),
                index.clone(),
                history.clone(),
    ...
    impl<'a> ResponseStream<'a> {
        ...
        fn emit_progress(&mut self, args: std::fmt::Arguments<'_>) -> io::Result<()> {
            self.ensure_started()?;
            self.stream.write_all(b"progress: ")?;
            self.stream.write_fmt(args)?;
            self.stream.write_all(b"\n")?;
            self.stream.flush()
        }

        fn write_body(&mut self, body: &str) -> io::Result<()> {
            self.ensure_started()?;
            self.stream.write_all(body.as_bytes())?;
            self.stream.flush()
        }

        fn write_err(&mut self, msg: &str) -> io::Result<()> {
            debug_assert!(
                !self.started,
                "ResponseStream::write_err called after OK was streamed",
            );
    ...
    fn write_err_if_unstarted(response: &mut ResponseStream<'_>, msg: &str) -> io::Result<()> {
        if response.started() {
            response.emit_progress(format_args!("error {msg}"))
        } else {
            response.write_err(msg)
        }
    }
    ...
    fn start_directory_monitor(
        ...
        let handle = thread::spawn(move || {
            if let Err(error) = monitor_directory(
                managed_dir,
                agent_home,
                index,
                history,
            ...
            match receiver.recv_timeout(Duration::from_millis(50)) {
                Ok(Ok(event)) => {
                    if event_affects_managed_subtree(&subtree, &event.paths) {
                        ...
                        let nested = ManagedSubtree::new(&managed_dir, &agent_home)
                            .nested_managed_relative_paths()?;
                        history.record_external_change(&managed_dir, &nested)?;
...
pub mod client {
    ...
    pub fn stream_ask<F>(prompt: &str, mut on_line: F) -> Result<(), Error>
    where
        F: FnMut(&str),
    {
        let cwd = std::env::current_dir().map_err(Error::Io)?;
        let request = format!("ASK\t{}\t{prompt}", cwd.display());
        let stream = connect()?;
        ...
        let read = reader.read_line(&mut header).map_err(Error::Io)?;
        if read == 0 {
        ...
        match header {
            ...
            "ERR" => {
                ...
                reader.read_to_string(&mut message).map_err(Error::Io)?;
                Err(Error::Supervisor(message.trim_end().to_string()))
            }
